"""
Translated from PyTorch version
https://github.com/madaan/minimal-text-diffusion/blob/main/src/modeling/diffusion/gaussian_diffusion.py
"""
...
def betas_for_alpha_bar_trunc_cosine(
...
class GaussianDiffusion:
    ...
    def training_losses(
        self, model, x_start_never_used, t, model_kwargs=None, noise=None
    ):
        """
        ...
        for i in indices:
            ...
            with th.no_grad():
                ...
                if langevin_func is not None:
                    out["t"] = t
                    out["img"] = img
                    out["sample"] = langevin_func(
                        x_t=out["sample"],
                        t=t,
                        label_ids=th.ones_like(t).to(out["img"].device),
                    )
                yield out
                img = out["sample"]
