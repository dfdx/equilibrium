...
class DDPMScheduler:
    ...
    def add_noise(self, x0, noise, timesteps):
        ...
        sqrt_one_minus_alphas_cumprod_t = jnp.take(
    ...
    def sample_x_t_prev(
        self, eps: jax.Array, x_t: jax.Array, timesteps: jax.Array, z: jax.Array
    ):
        """
        ...
        * z: jax.Array
        ...
        t = timesteps
        # coefficients
        alphas_t = jnp.take(self.alphas, t)[
            :, None, None
        ]  # perhaps only works for 3D inputs
        sqrt_one_minus_alphas_cumprod_t = jnp.take(
        ...
        sigmas_t = jnp.sqrt(
            betas_t
        )  # according to Section 3.2 in https://arxiv.org/pdf/2006.11239
        # sampling
        mu_t = (1 - jnp.sqrt(alphas_t)) * (
            x_t - (1 - alphas_t) / sqrt_one_minus_alphas_cumprod_t * eps
        )
